...
from movie.dataclasses import ImdbMovie, AiMovie
...
class FinderTests(APITestCase):
    ...
    def test_find_movie(self, mock_get_movies_from_imdb):
        mock_get_movies_from_imdb.return_value = [
            ImdbMovie(
                title="Shrek", imdb_id="12345", poster="http://poster.url", year="2001", type="movie"
            ),
        ]
    ...
    def test_create_watch_later(self):
        ...
        with self.assertNumQueries(3):
            response = self.client.post(
                reverse("watch_later_create"), data={"movie": self.movie.id}
            )
    ...
    def test_delete_watch_later(self):
        ...
        with self.assertNumQueries(2):
            response = self.client.delete(
                reverse("watch_later_destroy", kwargs={"pk": wl.movie.id})
            )
